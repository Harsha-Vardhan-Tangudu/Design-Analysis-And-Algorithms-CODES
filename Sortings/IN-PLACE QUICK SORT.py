...
def MAX_HEAPIFY(A, i, heap_size, comparisons, swaps, primitive_operations):
    ...
    if left <= heap_size:
        comparisons[0] += 1
        primitive_operations[0] += 1  # Increment primitive operations count (1 comparison)

        if A[left - 1] > A[largest - 1]:
            largest = left
            primitive_operations[0] += 1  # Increment primitive operations count (1 assignment)
...
# Function to perform in-place heap sort with counting comparisons and swaps
...
for size in test_sizes:
    ...
    # Measure running time for the best-case scenario
    start_time = time.time()

    # Perform Inplace Heap Sort with counting for the best-case scenario
    inplace_heapsort(best_case_array, comparisons, swaps, primitive_operations)  # Overall time complexity: O(n log n)

    # Calculate running time in milliseconds for the best-case scenario
    end_time = time.time()
    running_time_ms = (end_time - start_time) * 1000

    # Calculate memory usage for the best-case scenario
    memory_used_bytes = psutil.Process().memory_info().rss
    ...
    # Measure running time for the worst-case scenario
    start_time = time.time()

    # Perform Inplace Heap Sort with counting for the worst-case scenario
    inplace_heapsort(worst_case_array, comparisons, swaps, primitive_operations)  # Overall time complexity: O(n log n)

    # Calculate running time in milliseconds for the worst-case scenario
    end_time = time.time()
    running_time_ms = (end_time - start_time) * 1000

    # Calculate memory usage for the worst-case scenario
    memory_used_bytes = psutil.Process().memory_info().rss
